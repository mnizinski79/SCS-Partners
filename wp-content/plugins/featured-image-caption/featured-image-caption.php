...
function cc_featured_image_caption( $echo = true, $attribution = true ) {
	...
	if ( $echo ) {
		...
		if ( ! empty( $captiondata['caption_text'] ) ) {
			$caption = '<p class="' . $pluginid . '">' . $captiondata['caption_text'] . '</p>';
		}
		else {
			$caption = null;
		}
		...
		if ( ! empty( $captiondata['source_text'] ) && false != $attribution ) {
			...
			if ( ! empty( $captiondata['source_url'] ) ) {
				...
				if ( ! empty( $captiondata['new_window'] ) ) {
					$caption .= ' <a href="' . $captiondata['source_url'] . '" target="_blank" class="btn dark accent">' . $captiondata['source_text'] . '</a>';
				}
				else {
					$caption .= ' <a href="' . $captiondata['source_url'] . '" class="btn dark accent">' . $captiondata['source_text'] . '</a>';
				}
			}
